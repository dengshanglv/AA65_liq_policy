...
xnewd = kron(ones([1 NS]), qnew) + kron(omega.*theta, ones([1 NS]))...
    + kron((zmd+zld.*(1.+irate))./q, qnew./(1+gma_new));
xnewc = kron(ones([1 NS]), qnew) + kron((zmc+zlc.*(1.+irate))./q, qnew./(1+gma_new));
...
eqn6 = sum(reshape(focmc.*pr_mat,NS,NS));                             % FOC for md, when i==0
